...
#|(defun pdf2txt (file &optional (textfile (merge-pathnames (make-pathname :type "txt") file)))
  (unless (probe-file file)
    (format *error-output* "There is no file:~S." file)
    (return-from pdf2txt))
  (let ((cmd (format nil "\"C:\\Program Files\\d2txt141\\xdoc2txt.exe\" ~S" file)))
    (when (probe-file textfile)
      (delete-file textfile))
    (format t "Executing ~A ..." cmd)
    (if (eq textfile t)
        (excl:run-shell-command cmd)
      (excl:run-shell-command cmd :output textfile))))|#

(defun pdf2txt (file &optional (textfile (merge-pathnames (make-pathname :type "TXT" :case :common) file)))
  (unless (probe-file file)
    (format *error-output* "There is no file:~S." file)
    (return-from pdf2txt) )
  (let ((cmd (format nil "pdftotext ~S" file)))
    (when (probe-file textfile)
      (delete-file textfile) )
    (format t "~&Executing ~A => ~A ... " cmd textfile)
    (format t
            "~A~%"
            (if (zerop
